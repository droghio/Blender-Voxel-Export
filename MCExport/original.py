...
class VoxelizeOutput(bpy.types.Operator):
    ...
    def execute(self, context):        # execute() is called by blender when running the operator.
        ...
        for axis in range(3):
            ...
            """Currently all voxels are scanned 3 times, once in the x, y, and z. Their color is recorded."""
                if (axis != 2):
                    break
        ...
        if (len(selectedfaces) > 0):
            ...
            #Determine where there are voxels.
                ...
                for f in selectedfaces:
                    ...
                    try:
                        #Air spaces are undefined, not zero in this case.
                        if (volume[math.floor(v[0])][math.floor(v[1])][math.floor(v[2])] == baseMaterial):
                ...
                #print(volume)
                """Now that we have figured out all the faces perpendicular to our search axis, go through and figure out
                   where the voxels are. Each time we hit a face we flip whether we are placing, or removing voxels."""
                for x in range(int(max_coord[0])-int(min_coord[0])):
                    plane = []
                    for y in range(int(max_coord[1])-int(min_coord[1])):
                        strip = []
                        tracingblock = 0
                        lastMaterial = baseMaterial
                        for z in range(int(max_coord[2])-int(min_coord[2])):
                            ...
                            if (tracingblock == 1):
                                try:
                                    ...
                                    lastMaterial = volume[x+min_coord[0]][y+min_coord[1]][z+min_coord[2]]
                                except:
                                    strip.append(lastMaterial)
                            else:
                                strip.append(0)
                        plane.append(strip)
